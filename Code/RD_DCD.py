...
def RD_DCD(taskMap,workflowMap,vm_data,avg_compute_power=40):
    global AVG_VM_COMPUTE_POWER
    global relative_function_deadline
    AVG_VM_COMPUTE_POWER = avg_compute_power
        ...
        if(cur_top_element.type == MACHINE_RESERVE):
            ...
            for time_points in machines_not_reserved[cur_type]:
                if(abs(time_points - cur_time) < SPOT_TIME_WINDOW and spot_counter<SPOT_MACHINE_QUANTITY_THRESHOLD):
                    ...
                    new_vm.available_time = cur_time  
                    free_machines.append(new_vm)
                    R_D_S_Stats.total_spot_cost+= type_price_tup[1]
    ...
    total_cost=R_D_S_Stats.reserve_cost + R_D_S_Stats.ondemand_cost + R_D_S_Stats.total_spot_cost

    return "R+D_DCD",R_D_S_Stats.reserve_cost,R_D_S_Stats.ondemand_cost,R_D_S_Stats.total_spot_cost,total_cost,R_D_S_Stats.total_reward,R_D_S_Stats.total_reward-total_cost,total_deadlines_missed
